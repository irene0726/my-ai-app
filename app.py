                ...
                try:
                   prompt = f"""
                    ...
                    6. ⚔️ 主動攻防與風向引導作戰 (網軍實戰操盤)
                       - 【風向轉移】：如何設計留言，將討論焦點從「產品本身的缺陷」巧妙轉移到「個人的使用習慣、體質差異」或是「同類型保養品/產品的通病」？
                       - 【反向質疑】：針對該負評的邏輯漏洞，設計帶風向的留言來「合理質疑」原 PO 的動機 (例如：是否為競品惡意攻擊？是否未依正確指示使用？)。
                       - 【洗文稀釋策略】：針對 Dcard、PTT、Threads 或 BabyHome 等不同生態，建議後續一週內該用什麼主題的「正面/開箱文」來把這篇負評洗下去？
                       
                    7. ⚖️ 平台機制與長尾防護
                       - 評估透過版規檢舉下架的可行性，以及後續的 SEO 防護建議。
                    """
                    """
                    response = model.generate_content(prompt)
