            ...
            with st.spinner("🕵️‍♂️ AI 正在深度分析中，請稍候..."):
                try:
                    prompt = (
                        ...
                        "7. 📌 此產品於各論壇討論風向差異 (Threads、Dcard、PTT 等論壇網友在意的點有何不同？)\n"
...
# ------------------------------------------
with tab2:
    st.info("💡 **操作與訓練指南**：設定產業與人設條件，下方黑框會即時顯示「AI 詠唱指令」，幫助團隊邊做邊學。")
    
    col_ind, col_t, col_a = st.columns([1.5, 2, 2])
    with col_ind:
        industry = st.selectbox("🏢 產業類別：", ["醫美/診所", "保健食品", "美妝保養", "室內設計/裝潢", "月子中心/母嬰", "3C家電", "其他"])
    with col_t:
        treatment = st.text_input("📦 產品/服務名稱：", placeholder="例如：海芙音波、葉黃素、老屋翻新...")
    with col_a:
