    ...
    if st.button("🚀 開始負評拆彈"):
        if raw_reviews:
            with st.spinner("🚨 AI 危機處理專家正在擬定對策，請稍候..."):
                try:
                    prompt = f"""
                    你是一位資深的危機處理專家與公關顧問。客戶目前遭遇了以下網友的具體負評攻擊。
                    
                    以下是網友的原始負評內容：
                    「{raw_reviews}」

                    請針對上述的原始負評內容直擊核心，整理出以下5點「負評拆彈與回覆報告」：
                    1. 🌋 核心情緒與痛點拆解 (這位網友真正生氣、不滿的底層原因是什麼？)
                    2. 🛡️ 負評處理與緩頰切角建議 (請提供5個不具業配感、合理且客觀的反駁或護航觀點)
                    3. 🚨 後續延燒風險評估 (這篇負評有沒有可能引發更大規模的炎上？)
                    4. 🩹 官方公關回覆建議 (請擬定一段可以直接回覆給該網友的官方留言，語氣需誠懇、能安撫情緒且不推卸責任)
