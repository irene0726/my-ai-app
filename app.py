            ...
            with st.spinner("🤖 正在注入 Threads 演算法爆款邏輯..."):
                try:
                    prompt = (
                        ...
                        "【🏆 Threads 爆文必備法則 (嚴格遵守)】\n"
                        "1. 停止廢話：第一句話直接切入重點或情緒，字數越少越好，必須具備「滑到就想停下來看」的阻斷力。\n"
                        "2. 視覺排版：大量使用換行（Enter），每句話盡量不超過 15 個中文字，創造極致的閱讀順暢感與呼吸感。\n"
                        "3. 絕對擬真：禁用「大家覺得呢？」這種老派 IG 結尾。要使用非常口語的台灣網路用語（可適度加入：欸、真的瘋掉、笑死、求勸退、在線等）。\n"
                        "4. 留破綻/挖坑：結尾必須拋出一個「具體但兩難的選擇」或「極具爭議的提問」，誘發網友的『好為人師』心理，讓他們忍不住留言回覆。\n\n"
                        "【請產出以下內容】\n"
                        "請給我 3 個不同切入點的 Threads 串文版本：\n\n"
                        "🔹 版本 A：[直接給出改寫後的串文內容]\n"
